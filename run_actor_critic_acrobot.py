...
from untitled import PolicyGradientActorCritic
...
writer = None#tf.train.SummaryWriter("/tmp/{}-experiment-1".format(env_name))
...
for i_episode in range(MAX_EPISODES):
  ...
  for t in range(MAX_STEPS):
    #env.render()
    action, value = pg_reinforce.sampleAction(state[np.newaxis,:])
    next_state, reward, done, _ = env.step(action)

    total_rewards += reward
    reward = 5.0 if done else -0.1
    pg_reinforce.storeRollout(state, action, reward, value)
