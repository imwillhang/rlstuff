...
def LinearPolicy(states):
  hidden1 = layers.fully_connected(states, 20, weights_initializer=tf.random_normal_initializer())
  hidden2 = layers.fully_connected(hidden1, 20, weights_initializer=tf.random_normal_initializer())
  hidden3 = layers.fully_connected(hidden1, 20, weights_initializer=tf.random_normal_initializer())
  logits = layers.fully_connected(hidden3, 3,
    activation_fn=None,
    weights_initializer=tf.random_normal_initializer(),
    biases_initializer=None)
  pi = tf.nn.softmax(logits)
  actions = tf.squeeze(tf.multinomial(logits, 1))
  vf = layers.fully_connected(hidden3, 1, 
    activation_fn=None,
    weights_initializer=tf.random_normal_initializer(),
    biases_initializer=None)
...
class PolicyGradientActorCritic(object):
  ...
  def create_variables(self):
    ...
    with tf.name_scope("predict_actions"):
      ...
      with tf.variable_scope("actor_network"):
        self.policy_outputs, self.value_outputs = self.network(self.states)
      # with tf.variable_scope("critic_network"):
      #    = self.critic_network(self.states)
    ...
    with tf.name_scope("compute_pg_gradients"):
      ...
      with tf.variable_scope("actor_network", reuse=True):
        self.logprobs, self.estimated_values = self.network(self.states)
      ...
      self.entropy = -tf.reduce_mean(tf.reduce_sum(tf.nn.softmax(self.logprobs) * tf.nn.log_softmax(self.logprobs), axis=1))

      self.actor_loss         = self.pg_loss + self.reg_param * self.actor_reg_loss - 0.01 * self.entropy#
      ...
      self.pg_loss = self.pg_loss * self.advantages
      ...
      # self.critic_gradients = self.optimizer.compute_gradients(self.critic_loss, actor_network_variables)

      # collect all gradients
      # self.gradients = self.actor_gradients + self.critic_gradients
      self.loss = self.pg_loss + self.critic_loss * 0.5 - self.entropy * 0.01
      self.gradients = self.optimizer.compute_gradients(self.loss, actor_network_variables)
  ...
  def sampleAction(self, states):
    ...
    actions, values = self.session.run([self.predicted_actions, self.estimated_values], {self.states: states})
    return actions[0][0], values
  ...
  def updateModel(self):
    ...
    #   # future discounted rewa`rd from now on
    #   r = self.reward_buffer[t] + self.discount_factor * r
    #   discounted_rewards[t] = r

    value_with_v = np.array(self.value_buffer)

    discounted_rewards = discount(self.reward_buffer, self.discount_factor)
    advantage = discounted_rewards - value_with_v#self.reward_buffer + self.discount_factor * value_with_v[1:] - value_with_v[:-1]
    #advantage = discount(temporal_diff, self.discount_factor)
    # whether to calculate summaries
    #calculate_summaries = False#self.train_iteration % self.summary_every == 0 and self.summary_writer is not None
  ...
  def storeRollout(self, state, action, reward, val):
